...
it('"Wish Mart" 텍스트 로고를 클릭한 경우 "/" 경로로 navigate가 호출된다.', async () => {
...
describe('로그인이 된 경우', () => {
  ...
  it('장바구니(담긴 상품 수와 버큰)과 로그아웃 버튼(사용자 이름: "Miles")가 노출된다.', async () => {
    await render(<NavigationBar />);

    expect(screen.getByTestId('cart-icon')).toBeInTheDocument();
    expect(screen.getByText('2')).toBeInTheDocument();
    expect(
      await screen.findByRole('button', { name: 'Miles' }),
    ).toBeInTheDocument();
  });
  ...
  describe('로그아웃 버튼(사용자 이름: "Miles")을 클릭하는 경우', () => {
    let userEvent;
    beforeEach(async () => {
      const { user } = await render(<NavigationBar />);
      userEvent = user;

      const logoutBtn = await screen.findByRole('button', { name: 'Miles' });
